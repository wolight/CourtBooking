...
$.get("/booking/courts", function(data, status){
      let courts =  data.courts;
      let length = courts.length;
      let table = '<select style="width: 50%" name="courtName" id="courtName" required="true">';
      for(let i = 0; i < length; i++){
            let n = courts[i];
            table += '<option class="option">' + n['courtName'] +'</option>';
      }
      table += '</select><br>';
      $('#courtlist').append(table);
    });
});



// inspired by http://www.openjs.com/scripts/ui/calendar/
...
calendar = {
  ...
  showCalendar: function() {
    ...
    div.style.top=xy[1]+ height + 10 + "px";
    if(screen.width < 480){
        div.style.left = screen.width / 6 + "px";
    }
...
// Please acknowledge use of this code by including this header.
    ...
    function check() {
      $.ajax({
          ...
          statusCode : {
            400 : function(){alert("Choose another time!");}
          }
      });
    }


  function checkTime(field1, field2) {
    var errorMsg = "";

    if(field1.value != '') {
      if(field2.value != ''){
          if (field2.value - field1.value > 1){
            errorMsg = 'You can only book an hour at a time';
          }
          if(field2.value - field1.value < 0){
            errorMsg = 'Invalid booking';
          }
      }
    }
  ...
  function checkForm(form) {
    if(checkTime(form.starttime, form.endtime)) return false;
    return true;
  }
  ...
  function createBooking(){
      ...
      if(confirmation){
        $.ajax({
          method:"POST",
          url:'/booking?' + $(this).serialize(),
          dataType : "json",
          statusCode : {
            403 : function() {alert("Already Booked!\nChoose another one!");}
          }
      });
  }
